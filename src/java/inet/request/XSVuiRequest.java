...
import inet.bean.Event;
import inet.bean.EventJson;
import inet.bean.LichSuDuDoan;
import inet.bean.LichSuDuDoanJson;
import inet.bean.Register;
...
public class XSVuiRequest {
    ...
    public Register parserRegister(String mobile,String username,String password){
        try{
            Gson gson=new Gson();        
            String url=Contant.URL_XO_SO_VUI_REGISTER;
            ...
            if(string.contains("status")){                
                Register register=(Register)gson.fromJson(string, Register.class);                
                return register;
            }
        }catch(Exception e){
            System.out.println("-----------API.XOSO.WAP.VN--------------");
            System.out.println("=======XSVuiRequest=======>>parserRegister=======>>"+e.toString());        
        }                
        return null;
    }
    ...
    public List<XSVuiHistory> parserHistory(String mobile,String sday,String eday){
        try{
            Gson gson=new Gson();        
            String url=Contant.URL_XO_SO_VUI_HISTORY;            
            url=url.replace("mobile", mobile).replace("sday", sday).replace("eday", eday);            
...
//            }
    ...
    public static void pushDangKy(String i,String m){
        try{
            String url=Contant.URL_DANG_KY;
            url=url.replace("idinet", i);
            url=url.replace("MOBILE", m);
            String string=HttpURLRequest.sendGet(url);
        }catch(Exception e){}
    }
    ...
    public static Event getEvent(){
        try{
            Gson gson=new Gson();
            String url=Contant.URL_EVENT;
            String string=HttpURLRequest.sendGet(url);
            EventJson eventJson=(EventJson)gson.fromJson(string, EventJson.class);
            if(eventJson.getStatus()==0){
                return eventJson.getContent();
            }            
        }catch(Exception e){}
    ...
    public static String pushDuDoan(String uid,String evid,String de,String loto){
        try{
            String url=Contant.URL_DU_DOAN;
            url=url.replace("uid", uid);
            url=url.replace("evid", evid);
            url=url.replace("de", de);
            url=url.replace("lt", loto);
            String string=HttpURLRequest.sendGet(url);
            Gson gson=new Gson();
    ...
    public static List<LichSuDuDoan> getDuDoan(String uid,String evid,String page,String rowPerPage,String date){
        try{
            Gson gson=new Gson();
            String url=Contant.URL_LICH_SU_DU_DOAN;
            url=url.replace("uid", uid);
            url=url.replace("evid", evid);
            url=url.replace("PAGE", page);
            url=url.replace("ROW", rowPerPage);
            url=url.replace("today", date);
            String string=HttpURLRequest.sendGet(url);
            LichSuDuDoanJson lichSuDuDoanJson=(LichSuDuDoanJson)gson.fromJson(string, LichSuDuDoanJson.class);
            
            if(lichSuDuDoanJson.getStatus()==0){
                return lichSuDuDoanJson.getContent();
            }
    ...
    public static void main(String[] args) {
        List<LichSuDuDoan> lichSuDuDoan=XSVuiRequest.getDuDoan("1760631","841","1","10","24/02/2016");
        for (Iterator<LichSuDuDoan> it = lichSuDuDoan.iterator(); it.hasNext();) {
            LichSuDuDoan lichSuDuDoan1 = it.next();
            System.out.println(lichSuDuDoan1.getLotoBet());
        }
    }
